...
# NEED HELP CHANGING THE Q5 BREAKS
grid.arrange(ncol=2,
  ggplot()+
...
#need help with the breaks
grid.arrange(ncol=2,
  ggplot()+
    geom_sf(data = nhood, fill = "grey40") +
    geom_sf(data=rtt_17, aes(color=value_17))+
    scale_color_viridis()+
...
ggplot() + 
  ...
  scale_fill_viridis() +
  scale_alpha(range = c(0.00, 0.50), guide = FALSE) +
  labs(title = "Where homevalues increased") +
  theme(legend.position = "none")+
  mapTheme(),
...
ggplot() + 
  ...
  scale_fill_viridis() +
  scale_alpha(range = c(0.00, 0.35), guide = FALSE) +
  labs(title = "Where homevalues decreased") +
  theme(legend.position = "none")+
  mapTheme())
...
#Plot of all the point data - a little cluttered so maybe we just want to skip this
grid.arrange(ncol=3,
  ggplot()+
    geom_sf(data=boundary)+
    geom_sf(data=sanitation)+
    labs(title="Locations of Sanitation Violations")+
    mapTheme(),
  ggplot()+
    geom_sf(data=boundary)+
    geom_sf(data=service)+
    labs(title="Location of Service Requests")+
    mapTheme(),
  ggplot()+
    geom_sf(data=boundary)+
    geom_sf(data=vacant)+
    labs(title="Location of Vacant Lots")+
    mapTheme())
...
point features turned into nearest neighbor

DELETE THIS ONCE WE'VE CONFIRMED MY FEATURE PLOTS BELOW WILL WORK
